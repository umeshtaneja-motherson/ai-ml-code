...
dfnew1 = pd.read_csv('s3://convosight-dev-redshift/config/6_Job_facebookPostsTracker_insightsupload.csv')
#s3://convosight-dev-redshift/config/6_Config_facebookPostsTracker_insightsupload.csv 
...
for i in range(no_of_table):
    try:
        ...
        datasink1 = glueContext.write_dynamic_frame.from_jdbc_conf(frame = new_df, catalog_connection = "dynamodb_to_redshift", connection_options = {"dbtable":redshift_table_main, "database": "convosight_dev_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink1")
        ...
        for item in list_cols :
            ...
            datasink_subtable = glueContext.write_dynamic_frame.from_jdbc_conf(frame = new_df_1, catalog_connection = "dynamodb_to_redshift", connection_options = {"dbtable": sub_table_name, "database": "convosight_dev_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink2")
...
dfnew1.to_csv ('s3://convosight-dev-redshift/config/6_Job_facebookPostsTracker_insightsupload.csv', index = False, header=True)
s3_target = "s3://convosight-dev-redshift/status_report/6_Config_facebookPostsTracker_insightsupload/"
