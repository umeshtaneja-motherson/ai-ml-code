...
dfnew1 = pd.read_csv('s3://convosight-dev-redshift/config/2_Job_SM.csv')
...
for i in range(no_of_table):
    try:
        ...
        if dynamodb_tablename == 'bd-convosight-dev-groupProfile':
            dfirst = dfirst.withColumn("campaignsDetails", to_json("campaignsDetails"))
        dfirst.show(5)
        ...
        if  updatetime == 'not-present':
            ...
            if(dynamodb_tablename != 'bd-convosight-dev-brandcrmleads'):
        ...
        datasink_temp = glueContext.write_dynamic_frame.from_jdbc_conf(frame = temp_new_df, catalog_connection = "dynamodb_to_redshift", connection_options = {"preactions":pre_query1,"dbtable":"bd_staging_table_2", "database": "convosight_dev_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink_temp")# added
        ...
        for item in list_cols :
            ...
            datasink_temp_subtable = glueContext.write_dynamic_frame.from_jdbc_conf(frame = temp_new_df, catalog_connection = "dynamodb_to_redshift", connection_options = {"preactions":pre_query2,"dbtable":"bd_staging_table_2", "database": "convosight_dev_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink_temp_subtable")# added
...
dfnew1.to_csv ('s3://convosight-dev-redshift/config/2_Job_SM.csv', index = False, header=True)
s3_target = "s3://convosight-dev-redshift/status_report/2_Job_SM/"
