...
dfnew1 = pd.read_csv('s3://convosight-dev-redshift/config/4_Job_groupMetrics.csv')
...
for i in range(no_of_table):
    try:
        ...
        pre_query1 = "drop table if exists bd_staging_table_4;create table if not exists bd_staging_table_4(key_id varchar(100) not null);"
        print(pre_query1)

        if  updatetime == 'not-present':
            post_query1 = "begin;DELETE FROM {} WHERE createddatetime IN (SELECT createddatetime FROM (SELECT createddatetime, dense_rank() OVER (PARTITION BY key_id ORDER BY createddatetime DESC) rn FROM {})t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_4)); drop table bd_staging_table_4;end;".format(redshift_table_main,redshift_table_main)#added
        else :
            post_query1 = "begin;DELETE FROM {} WHERE updateddatetime IN (SELECT updateddatetime FROM (SELECT updateddatetime, dense_rank() OVER (PARTITION BY key_id ORDER BY updateddatetime DESC) rn FROM {})t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_4)); drop table bd_staging_table_4;end;".format(redshift_table_main,redshift_table_main)#added    
        print(post_query1)#"postactions":post_query1
        
        datasink_temp = glueContext.write_dynamic_frame.from_jdbc_conf(frame = temp_new_df, catalog_connection = "dynamodb_to_redshift", connection_options = {"preactions":pre_query1,"dbtable":"bd_staging_table_4", "database": "convosight_dev_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink_temp")# added
        ...
        for item in list_cols :
            ...
            pre_query2 = "drop table if exists bd_staging_table_4;create table if not exists bd_staging_table_4(key_id varchar(100) not null);"
            print(pre_query2)
            if  updatetime == 'not-present':
                post_query2 = "begin;DELETE FROM {} WHERE createddatetime IN (SELECT createddatetime FROM (SELECT createddatetime, dense_rank() OVER (PARTITION BY key_id ORDER BY createddatetime DESC) rn FROM {})t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_4)); drop table bd_staging_table_4;end;".format(sub_table_name,sub_table_name)#added
            else :
                post_query2 = "begin;DELETE FROM {} WHERE updateddatetime IN (SELECT updateddatetime FROM (SELECT updateddatetime, dense_rank() OVER (PARTITION BY key_id ORDER BY updateddatetime DESC) rn FROM {})t WHERE rn > 1 and key_id in (SELECT key_id FROM bd_staging_table_4)); drop table bd_staging_table_4;end;".format(sub_table_name,sub_table_name)#added    
            print(post_query2)#"postactions":post_query1
            
            datasink_temp_subtable = glueContext.write_dynamic_frame.from_jdbc_conf(frame = temp_new_df, catalog_connection = "dynamodb_to_redshift", connection_options = {"preactions":pre_query2,"dbtable":"bd_staging_table_4", "database": "convosight_dev_dynamo_data"}, redshift_tmp_dir = args["TempDir"], transformation_ctx = "datasink_temp_subtable")# added
...
dfnew1.to_csv ('s3://convosight-dev-redshift/config/4_Job_groupMetrics.csv', index = False, header=True)
s3_target = "s3://convosight-dev-redshift/status_report/4_Job_groupMetrics/"
